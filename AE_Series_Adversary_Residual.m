...
%% Discriminator
paramsDis1.FCW1 = dlarray(initializeGaussian([512,prod(settings.image_size)],.02));
paramsDis1.FCb1 = dlarray(zeros(512,1,'single'));
paramsDis1.FCW2 = dlarray(initializeGaussian([256,512]));
paramsDis1.FCb2 = dlarray(zeros(256,1,'single'));
paramsDis1.FCW3 = dlarray(initializeGaussian([1,256]));
paramsDis1.FCb3 = dlarray(zeros(1,1,'single'));
paramsDis2.FCW1 = dlarray(initializeGaussian([512,prod(settings.image_size)],.02));
paramsDis2.FCb1 = dlarray(zeros(512,1,'single'));
paramsDis2.FCW2 = dlarray(initializeGaussian([256,512]));
paramsDis2.FCb2 = dlarray(zeros(256,1,'single'));
paramsDis2.FCW3 = dlarray(initializeGaussian([1,256]));
paramsDis2.FCb3 = dlarray(zeros(1,1,'single'));
paramsDis3.FCW1 = dlarray(initializeGaussian([512,prod(settings.image_size)],.02));
paramsDis3.FCb1 = dlarray(zeros(512,1,'single'));
paramsDis3.FCW2 = dlarray(initializeGaussian([256,512]));
paramsDis3.FCb2 = dlarray(zeros(256,1,'single'));
paramsDis3.FCW3 = dlarray(initializeGaussian([1,256]));
paramsDis3.FCb3 = dlarray(zeros(1,1,'single'));
paramsDis4.FCW1 = dlarray(initializeGaussian([512,prod(settings.image_size)],.02));
paramsDis4.FCb1 = dlarray(zeros(512,1,'single'));
paramsDis4.FCW2 = dlarray(initializeGaussian([256,512]));
paramsDis4.FCb2 = dlarray(zeros(256,1,'single'));
paramsDis4.FCW3 = dlarray(initializeGaussian([1,256]));
paramsDis4.FCb3 = dlarray(zeros(1,1,'single'));
paramsDis5.FCW1 = dlarray(initializeGaussian([512,prod(settings.image_size)],.02));
paramsDis5.FCb1 = dlarray(zeros(512,1,'single'));
paramsDis5.FCW2 = dlarray(initializeGaussian([256,512]));
paramsDis5.FCb2 = dlarray(zeros(256,1,'single'));
paramsDis5.FCW3 = dlarray(initializeGaussian([1,256]));
paramsDis5.FCb3 = dlarray(zeros(1,1,'single'));
...
while ~out
    ...
    for i=1:numIterations
        ...
        [GradEn1,GradDe1,GradDis1] = dlfeval(@modelGradients1,XBatch1,paramsEn1,paramsDe1,paramsDis1);

        % Update Discriminator network parameters
        [paramsDis1,avgG1.Dis,avgGS1.Dis] = adamupdate(paramsDis1,GradDis1,avgG1.Dis,avgGS1.Dis,global_iter,settings.lrD,settings.beta1,settings.beta2);
...
while ~out
    ...
    for i=1:numIterations
        ...
        [GradDe2,GradDis2] = dlfeval(@modelGradients2,XBatch1,XBatch2,paramsDe2,paramsDis2);
     
        % Update Discriminator network parameters
        [paramsDis2,avgG2.Dis,avgGS2.Dis] = adamupdate(paramsDis2,GradDis2,avgG2.Dis,avgGS2.Dis,global_iter,settings.lrD,settings.beta1,settings.beta2);
...
while ~out
    ...
    for i=1:numIterations
        ...
        [GradDe3,GradDis3] = dlfeval(@modelGradients3,XBatch1,XBatch2,paramsDe3,paramsDis3);
     
        % Update Discriminator network parameters
        [paramsDis3,avgG3.Dis,avgGS3.Dis] = adamupdate(paramsDis3,GradDis3,avgG3.Dis,avgGS3.Dis,global_iter,settings.lrD,settings.beta1,settings.beta2);
    ...
    if epoch == settings.maxepochs-1
        trainR4 = trainX1-trainX4;
        tp = trainR4 > +1;
        tn = trainR4 < -1;
        trainR4 = trainR4 .* (1-tp) .* (1-tn) + tp - tn; 
    end
...
while ~out
    ...
    trainXshuffle1 = trainR4(:,shuffleid);
    ...
    for i=1:numIterations
        ...
        [GradDe4,GradDis4] = dlfeval(@modelGradients4,XBatch1,XBatch2,paramsDe4,paramsDis4);
     
        % Update Discriminator network parameters
        [paramsDis4,avgG4.Dis,avgGS4.Dis] = adamupdate(paramsDis4,GradDis4,avgG4.Dis,avgGS4.Dis,global_iter,settings.lrD,settings.beta1,settings.beta2);
    ...
    trainX5 = trainX5+trainX4;
    tp = trainX5 > +1;
    tn = trainX5 < -1;
    trainX5 = trainX5 .* (1-tp) .* (1-tn) + tp - tn;
    if epoch == settings.maxepochs-1
        trainR5 = trainX1-trainX5;
        tp = trainR5 > +1;
        tn = trainR5 < -1;
        trainR5 = trainR5 .* (1-tp) .* (1-tn) + tp - tn;
    end
    disp("Serial4 SSIM (train)="+gatext(my_ssim(trainX5,trainX1)));
    
    % testX5
    testX5 = Decoder(testX4,paramsDe4);
    testX5 = testX5+testX4;
    tp = testX5 > +1;
    tn = testX5 < -1;
    testX5 = testX5 .* (1-tp) .* (1-tn) + tp - tn; 
...
while ~out
    ...
    trainXshuffle1 = trainR5(:,shuffleid);
    ...
    for i=1:numIterations
        ...
        [GradDe5,GradDis5] = dlfeval(@modelGradients5,XBatch1,XBatch2,paramsDe5,paramsDis5);
     
        % Update Discriminator network parameters
        [paramsDis5,avgG5.Dis,avgGS5.Dis] = adamupdate(paramsDis5,GradDis5,avgG5.Dis,avgGS5.Dis,global_iter,settings.lrD,settings.beta1,settings.beta2);
    ...
    trainX6 = trainX6+trainX5;
    tp = trainX6 > +1;
    tn = trainX6 < -1;
    trainX6 = trainX6 .* (1-tp) .* (1-tn) + tp - tn;
    if epoch == settings.maxepochs-1
        trainR6 = trainX1-trainX6;
        tp = trainR6 > +1;
        tn = trainR6 < -1;
        trainR6 = trainR6 .* (1-tp) .* (1-tn) + tp - tn; 
    end
    disp("Serial5 SSIM (train)="+gatext(my_ssim(trainX6,trainX1)));
    
    % testX6
    testX6 = Decoder(testX5,paramsDe5);
    testX6 = testX6+testX5;
    tp = testX6 > +1;
    tn = testX6 < -1;
    testX6 = testX6 .* (1-tp) .* (1-tn) + tp - tn; 
...
%% model Gradients 1
function [GradEn,GradDe,GradDis]=modelGradients1(x,paramsEn,paramsDe,paramsDis)
y = Encoder(x,paramsEn);
x_ = Decoder(y,paramsDe);
input_fake = x_;
input_real = x;
d_output_fake = Discriminator(input_fake,paramsDis);
d_output_real = Discriminator(input_real,paramsDis);
d_loss = -.5*mean(log(d_output_real+eps)+log(1-d_output_fake+eps));
g_loss = .999*mean(mean(.5*(x_-x).^2,1))+.001*mean(log(1-d_output_fake+eps));
[GradEn,GradDe] = dlgradient(g_loss,paramsEn,paramsDe,'RetainData',true);
GradDis = dlgradient(d_loss,paramsDis);
end

%% model Gradients 2
function [GradDe,GradDis]=modelGradients2(xr,x,paramsDe,paramsDis)
...
d_output_real = Discriminator(input_real,paramsDis);
d_loss = -.5*mean(log(d_output_real+eps)+log(1-d_output_fake+eps));
g_loss = .999*mean(mean(.5*(x_-xr).^2,1))+.001*mean(log(1-d_output_fake+eps));
GradDe = dlgradient(g_loss,paramsDe,'RetainData',true);
GradDis = dlgradient(d_loss,paramsDis);
end

%% model Gradients 3
function [GradDe,GradDis]=modelGradients3(xr,x,paramsDe,paramsDis)
x_ = Decoder(x,paramsDe);
input_fake = x_;
input_real = xr;
d_output_fake = Discriminator(input_fake,paramsDis);
...
%% model Gradients 4
function [GradDe,GradDis]=modelGradients4(xr,x,paramsDe,paramsDis)
x_ = Decoder(x,paramsDe);
input_fake = x_;
input_real = xr;
d_output_fake = Discriminator(input_fake,paramsDis);
...
%% model Gradients 5
function [GradDe,GradDis]=modelGradients5(xr,x,paramsDe,paramsDis)
x_ = Decoder(x,paramsDe);
input_fake = x_;
input_real = xr;
d_output_fake = Discriminator(input_fake,paramsDis);
